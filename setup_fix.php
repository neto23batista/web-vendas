...
/* ── NAV: esconde textos, mantém ícones ── */
@media(max-width:700px){
  /* Header logo: esconde textos extras, mantém logo+ícone */
  .header .logo > span:not(:first-child){ display:none; }
  .header .logo > button,
  .header .logo > .auto-update-badge { display:none !important; }

  /* Nav buttons: todos viram ícone only */
  .nav-buttons .btn > i ~ * { display:none; }

  /* Stats: 5 cards → linha de 2 + linha de 3 */
  .stats-grid {
...
// Remove patch anterior se já existir
$atual = preg_replace('/\/\* ═+\s*MOBILE FIX PATCH.*$/s', '', $atual);
$atual = rtrim($atual) . "
" . $css_patch;
file_put_contents($css_file, $atual);

// ── 2. PATCH painel_dono.php — banner com classe ──
$painel = __DIR__ . '/painel_dono.php';
