...
if (schema_componentes_pendentes($conn, ['estoque'])) {
    redirecionar(
        'migracoes.php',
        'Existem migracoes pendentes de estoque. Execute-as antes de usar este modulo.',
        'erro'
    );
}
...
// â”€â”€ AUTO-MIGRAÃ‡ÃƒO â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
$acoes_post_estoque = ['acao_entrada', 'acao_saida', 'acao_ajuste', 'acao_transferencia', 'acao_limites'];
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    foreach ($acoes_post_estoque as $acao_post) {
...
// â”€â”€ ENTRADA â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
if (isset($_POST['acao_entrada'])) {
    ...
    try {
        estoque_registrar_entrada($conn, $id, $qtd, $motivo, $id_admin);
    ...
    if ($prod) {
        ...
        redirecionar('estoque.php',"âœ… Entrada de $qtd unidade(s) registrada!");
    }
}

// â”€â”€ SAÃDA â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
if (isset($_POST['acao_saida'])) {
    $id = (int)($_POST['id_produto'] ?? 0);
    $qtd = max(1, (int)($_POST['quantidade'] ?? 0));
    $motivo = sanitizar_texto($_POST['motivo'] ?? 'Saida de estoque');
    try {
        estoque_registrar_saida($conn, $id, $qtd, $motivo, $id_admin);
        redirecionar('estoque.php', "Saida de $qtd unidade(s) registrada.");
    } catch (Throwable $e) {
        redirecionar('estoque.php', $e->getMessage(), 'erro');
    }

    $id = (int)$_POST['id_produto']; $qtd = max(1,(int)$_POST['quantidade']);
    $motivo = sanitizar_texto($_POST['motivo'] ?? 'SaÃ­da de estoque');
    ...
    if ($prod) {
        ...
        $stmt=$conn->prepare("INSERT INTO movimentacoes_estoque (id_produto,tipo,quantidade,estoque_anterior,estoque_novo,motivo,id_usuario) VALUES (?,?,?,?,?,?,?)");
        $stmt->bind_param("isiissi",$id,$tipo,$qtd,$antes,$depois,$motivo,$id_admin); $stmt->execute(); $stmt->close();
        redirecionar('estoque.php',"âœ… SaÃ­da de $qtd unidade(s) registrada!");
    }
}

// â”€â”€ AJUSTE â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
if (isset($_POST['acao_ajuste'])) {
    $id = (int)($_POST['id_produto'] ?? 0);
    $novo = max(0, (int)($_POST['novo_estoque'] ?? 0));
    $motivo = sanitizar_texto($_POST['motivo'] ?? 'Ajuste de inventario');
    try {
        estoque_registrar_ajuste($conn, $id, $novo, $motivo, $id_admin);
    ...
    $motivo=sanitizar_texto($_POST['motivo']??'Ajuste de inventÃ¡rio');
    $stmt=$conn->prepare("SELECT estoque_atual FROM produtos WHERE id=?"); $stmt->bind_param("i",$id); $stmt->execute();
    $prod=$stmt->get_result()->fetch_assoc(); $stmt->close();
    if ($prod) {
        ...
        redirecionar('estoque.php',"âœ… Estoque ajustado para $novo unidade(s)!");
    }
}

// â”€â”€ TRANSFERÃŠNCIA â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
if (isset($_POST['acao_transferencia'])) {
    $id = (int)($_POST['id_produto'] ?? 0);
    $qtd = max(1, (int)($_POST['quantidade'] ?? 0));
    $orig = sanitizar_texto($_POST['localizacao_origem'] ?? '');
    $dest = sanitizar_texto($_POST['localizacao_destino'] ?? '');
    $motivo = sanitizar_texto($_POST['motivo'] ?? "Transferencia: $orig -> $dest");
    try {
        estoque_registrar_transferencia($conn, $id, $qtd, $motivo, $orig, $dest, $id_admin);
        redirecionar('estoque.php', "Transferencia registrada: $orig -> $dest.");
    } catch (Throwable $e) {
        redirecionar('estoque.php', $e->getMessage(), 'erro');
    }
    ...
    $motivo=sanitizar_texto($_POST['motivo']??"TransferÃªncia: $orig â†’ $dest");
    $stmt=$conn->prepare("SELECT estoque_atual FROM produtos WHERE id=?"); $stmt->bind_param("i",$id); $stmt->execute();
    $prod=$stmt->get_result()->fetch_assoc(); $stmt->close();
    if ($prod) {
        ...
        redirecionar('estoque.php',"âœ… TransferÃªncia: $orig â†’ $dest!");
    }
}

// â”€â”€ LIMITES â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
if (isset($_POST['acao_limites'])) {
    $id = (int)($_POST['id_produto'] ?? 0);
    $min = max(0, (int)($_POST['estoque_minimo'] ?? 0));
    $max = max(1, (int)($_POST['estoque_maximo'] ?? 1));
    $unidade = sanitizar_texto($_POST['unidade'] ?? 'un');
    $loc = sanitizar_texto($_POST['localizacao'] ?? '');
    try {
        estoque_atualizar_limites($conn, $id, $min, $max, $unidade, $loc);
        redirecionar('estoque.php', 'Configuracoes atualizadas.');
    } catch (Throwable $e) {
        redirecionar('estoque.php', $e->getMessage(), 'erro');
    }
    ...
    redirecionar('estoque.php',"âœ… ConfiguraÃ§Ãµes atualizadas!");
}

// â”€â”€ LEITURA â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
<div class="card" id="historico">
    ...
    <div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:20px;">
        ...
        <span style="font-size:12px;color:var(--text3);display:flex;align-items:center;gap:6px;padding:7px 14px;background:var(--surface2);border-radius:var(--radius-full);">
            <?= htmlspecialchars($np['nome']??'') ?> <a href="estoque.php#historico" style="color:var(--danger);">âœ•</a>
        </span>
        <?php endif; ?>
    </div>
